layout := '10 1E 2C 11 1F 2D 12 20 2E 13 21 2F '   .
          '14 22 30 15 23 31 16 24 32 17 25 33 '   .
          '18 26 34 19 27 35 1A 28 136 1B 2B 148 ' .
          '1C' ; kbdlayout.info/kbdusx/scancodes
  ...
  LShift Up:: (A.isBend) ? bend(2, 1)  : 0
   Space Up:: (Z.palmMute) or strum(0)
      #LAlt:: A.isLatch := !A.isLatch, tip()
       RAlt:: A.isSustain := !A.isSustain, tip()
...
A := { isActive: 1,   bendRange:  2, velocityA: 110,
      isSustain: 0,    midiPort:  0, velocityB:  90,
        isLatch: 0, midiChannel:  0,  cc1Value:   0,
         isBend: 1,   firstNote: 40,  cc3Value:   0, }
...
DllCall(wm 'Open', u '*', &midiOut:=0, u, A.midiPort, u,0,u,0,u,0) and ExitApp()

HotIf (*) => A.isActive
for sc in StrSplit(layout, ' ')
...
(tip() => TrayTip(!A.isActive ? '' :
  ...
  '`n' (A.isLatch ? 'latch' : 'hold') ' ' (A.isSustain ? '✅' : '❌')
...
key(k, isDown, *) {
  if isDown {
    if pressedKeys[k]
      return
    pressedKeys[k] := 1
    (A.isSustain and A.isLatch and !Z.anyKey) and strum(0)
    Z.anyKey++
